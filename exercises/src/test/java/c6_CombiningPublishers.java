...
/**
 * In this important chapter we are going to cover different ways of combining
 * publishers.
 *
 * Read first:
 *
...
public class c6_CombiningPublishers extends CombiningPublishersBase {
         ...
         * Once connection is established you will be able to collect messages from
         * stream.
         *
        ...
        /**
         * Join results from services `numberService1()` and `numberService2()`
         * end-to-end.
         * First `numberService1` emits elements and then `numberService2`. (no
         * interleaving)
         *
        ...
        /**
         * You are writing software for broker house. You can retrieve current stock
         * prices by calling either
        ...
        public void mail_box_switcher() {
                // todo: feel free to change code as you need
                // Flux<Message> myMail = Flux.concat(
                // mailBoxPrimary().takeWhile(message -> !message.metaData.equals("spam")),
                // mailBoxSecondary());

                Flux<Message> myMail = mailBoxPrimary().switchOnFirst((first, mailBoxPrimaryRest) -> {
                        if (first.get().metaData.equals("spam")) {
                                return mailBoxSecondary();
                        }
                        return mailBoxPrimaryRest;
                });
         ...
         * When user types in a text box results should appear in near real-time with
         * each keystroke.
         *
         ...
         * like `when`,`and`,`then`...
         * If all operations have been executed successfully return boolean value
         * `true`.
         */
        @Test
        ...
        /**
         * Execute all tasks sequentially and after each task have been executed, commit
         * task changes. Don't lose id's of
        ...
        public void acid_durability() {
                ...
                // Flux.usingWhen(tasksToExecute().concatMap(Function.identity()), taskId ->
                // commitTask(taskId).thenReturn(taskId), Mono::just);
                // tasksToExecute();
                // commitTask(null);
                Flux<String> committedTasksIds = tasksToExecute().concatMap(MonoTaskId -> Mono.usingWhen(MonoTaskId,
                                taskId -> commitTask(taskId).thenReturn(taskId), Mono::just));
        ...
        /**
         * News have come that Microsoft is buying Blizzard and there will be a major
         * merger.
         * Merge two companies, so they may still produce titles in individual pace but
         * as a single company.
         */
        @Test
        ...
        /**
         * When `chooseSource()` method is used, based on current value of sourceRef,
         * decide which source should be used.
         */
         ...
         * Open a connection to a streaming service and after all elements have been
         * consumed,
        ...
        public void cleanup() {
                ...
                Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(), x -> x,
                                x -> StreamingConnection.closeConnection());

                // don't change below this line
                StepVerifier.create(stream).then(() -> Assertions.assertTrue(StreamingConnection.isOpen.get()))
